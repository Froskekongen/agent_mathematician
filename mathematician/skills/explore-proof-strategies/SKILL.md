...
Find a useful proof architecture without pretending to certify it. Apply the
shared [rigor chain](../research-mathematics/references/rigor.md) to every
mathematical status claim. Default to a conversation-only report; for an
authorized writable theory, first read the shared
...
candidate proof. For writable work, retain only the selected architecture,
decisive evidence, immediate obligation, and reusable parked or rejected
routes under the memory protocol.
...
and every candidate proof remains visibly uncertified. Recommend
